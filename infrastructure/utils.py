...
def calculate_mean_prediction_error(env, action_sequence, models, data_statistics):
    ...
    for ac in action_sequence:
        ...
        # Get predicted next state using the model
...
def perform_actions(env, actions):
    ...
    successes, failures, crashes, overtimes, destroy = [], [], [], [], []
    steps = 0
    for ac in actions:
        obs.append(ob)
        acs.append(ac)
        ...
        failure = reward['failure']
        overtime = reward['overtime']
        destroyed = reward['destroy']
        next_obs.append(ob)
        rewards.append(rew)
    ...
    return Path(obs, image_obs, acs, rewards, next_obs, terminals, crashes, successes, failures, overtimes, destroy)
...
def sample_trajectory(env, policy, max_path_length, render=False, render_mode=('rgb_array')):
    ...
    while True:
        ...
        ac = policy.get_action(ob, steps, collisions)
        # print('ac: ', ac)
        ac = ac[0]
        acs.append(ac)

        # take that action and record results
        ob, rew, done, word = env.step(ac)
        ...
        success = rew['success']
        failure = rew['failure']
        overtime = rew['overtime']
        destroyed = rew['destroy']
        crashes.append(crash)
        successes.append(success)
        failures.append(failure)
        overtimes.append(overtime)
...
def Path(obs, image_obs, acs, rewards, next_obs, terminals, crashes, successes, failures, overtimes, destroy):
    """
    ...
    return {"observation": np.array(obs, dtype=np.float32),
            ...
            "destroy": np.array(destroy, dtype=np.float32)
            }
...
def convert_listofrollouts(paths):
    ...
    concatenated_rewards = np.concatenate(
        [np.squeeze(path["reward"]) for path in paths])
