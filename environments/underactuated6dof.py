...
Summary
    This environment is a 6DOF under-actuated ARPOD environment. 
...
class UnderactuatedDocking6DOF(gym.Env):
    ...
    def __init__(self, logdir=None):
        ...
        self.action_space = spaces.Box(np.array([-self.u_mag, -self.u_mag, -self.u_mag, self.fx_mag]),\
                                       np.array([self.u_mag, self.u_mag, self.u_mag, self.fx_mag]), dtype=np.float32)
    ...
    def step(self, action):
        ...
        action = np.clip(action, [-self.u_mag, -self.u_mag, -self.u_mag, -self.fx_mag],\
                                 [self.u_mag, self.u_mag, self.u_mag, self.fx_mag])

        # Extract current state and action data
        x, y, z, vx, vy, vz, mu1, mu2, mu3, w1, w2, w3, nu1, nu2, nu3 = self.state
        self.u1, self.u2, self.u3, self.fx = action
