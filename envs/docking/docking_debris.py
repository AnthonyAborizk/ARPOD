...
from envs.docking.rendering_debris import DockingRender as render
...
class SpacecraftDocking(gym.Env):
    ...
    def __init__(self):
        ...
        #  OBSTACLES
        self.num_obs = 10
        self.position_obstacle = 1500  # m relative from chief
    ...
    def reset(self):  # called before each episode
        ...
        # first two components are const x and y velocity
        self.obstacle = np.array([8, .2])
        for _ in range(self.num_obs):
            ang = self.np_random.uniform(low=2*math.pi/3, high=4*math.pi/3)
            self.x_obstacle = self.position_obstacle*math.cos(ang)  # m
            self.y_obstacle = self.position_obstacle*math.sin(ang)  # m
            self.obstacle = np.append(self.obstacle,
                                      np.array([self.x_obstacle, self.y_obstacle]))

        return self.state

    def mpc_reward(self, observations, actions, steps, collision):
        ...
        ans = []  # array of debris distances from deputy
        debris = self.obstacle[2:]
        sat = np.array([xx, yy])
        for i in range(self.num_obs):
            debris = np.array([self.obstacle[2+i*2], self.obstacle[3+i*2]])
            wut = debris - sat[:, i]
            ans.append(np.sqrt(wut[0]**2 + wut[1]**2))
        ...
        for i in range(xx.shape[0]):
            ...
            vH_min = 1/2 * self.n * rH - self.vel_threshold  # Min Velocity
                ...
                or collision >= self.max_collisions
    ...
    def get_reward(self, observations, actions):
        ...
        ans = []
        stuff = self.obstacle[2:]
        for i in range(self.num_obs):
            stuff = np.array([self.obstacle[2+i*2], self.obstacle[3+i*2]])
            wut = stuff - self.state[:2]
            ans.append(np.sqrt(wut[0]**2 + wut[1]**2))
        rew = []
        for i in range(xx.shape[0]):
            x = xx[i]
            y = yy[i]
                ...
                or self.collision >= self.max_collisions
            )
            rH = np.sqrt(x**2 + y**2)
            ...
            elif self.collision >= self.max_collisions:
                reward = -1
                self.destroyed += 1
    ...
    def step(self, action):
        ...
        x_dot_obs = self.obstacle[0]
        y_dot_obs = self.obstacle[1]

        for i in range(self.num_obs):
            self.obstacle[2+2*i] = self.obstacle[2+2*i] + x_dot_obs * self.tau
            self.obstacle[2*i+3] = self.obstacle[2*i+3] + y_dot_obs * self.tau
